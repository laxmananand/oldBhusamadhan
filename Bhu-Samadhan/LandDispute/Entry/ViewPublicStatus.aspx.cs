...
public partial class LandDispute_Entry_Unfinalize : System.Web.UI.Page
{
    ...
    private void GV_GetThanaDeatilsPageWise(int pageIndex)
    {
        ...
        try
        {
            ...
            p[5] = new SqlParameter("@CUUser", Session["UserID"].ToString());
            p[6] = new SqlParameter("@DistrictCode", Session["District_Code"].ToString() == "" ? "0" : Session["District_Code"].ToString());
            p[7] = new SqlParameter("@Subdivision", Session["Sub_DivCode"].ToString() == "" ? "0" : Session["Sub_DivCode"].ToString());
            p[8] = new SqlParameter("@BlockCode", Session["Block_Code"].ToString() == "" ? "0" : Session["Block_Code"].ToString());





            dt = clsData.GetDataTableWithProc("usp_GetSaveDataUnFinalizeAllUserPublic", p);
            int totalRecord = 0;
            if (p[2].Value != null)
            {
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string strFilePath = "PublicInformationEntry.aspx?RegId=" + UrlRedirect;
    ...
    protected void lnkAddMeeting_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("AddMettingApplicationPublic.aspx?RegId=" + UrlRedirect, false);
        }
        catch (Exception ex)
        {
            Response.Write(ex.Message.ToString());
    ...
    protected void lnkStaus_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("AddStatusApplicationPublic.aspx?RegId=" + UrlRedirect, false);
        }
        catch (Exception ex)
        {
            Response.Write(ex.Message.ToString());
